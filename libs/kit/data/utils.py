...
def set_df_cols_to(df, cols, values):
    """Set the values of the specified columns to the given value"""
    
    for i, col in enumerate(cols):
        value = values[i] if isinstance(values, (list, tuple)) else values
        df[col] = value
    return df


def cast_df_cols_to(df, cols, dtype, fillna=None):
...
def invert_dict(d):
    ...
    the values of the input dictionary become the keys of the output dictionary
    the keys of the input dictionary become the values in a list of the output dictionary
